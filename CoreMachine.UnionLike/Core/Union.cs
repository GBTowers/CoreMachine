...
{
	public ArityMember[] ArityMembers { get; } = Enumerable.Range(1, Arity).Select(i => new ArityMember(i)).ToArray(); 
	...
	public string WhereClauses =>
		$"where T : {Declaration} " + ArityMembers.JoinSelect(m => $"where {m.Name} : T", " ");
}
